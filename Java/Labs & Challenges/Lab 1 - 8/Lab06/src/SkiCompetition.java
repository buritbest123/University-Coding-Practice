...
public class SkiCompetition {
    ...
    public List<Competitor> getGoldMedal(List<Competitor> competitors, int round){
        //TODO: Code HERE
        List<Competitor> top_three = new ArrayList<>();
        //System.out.println(competitors.size());
        if(round != 0) {
            for (Competitor competitor : competitors) {
                if (competitor.getFinalScore(round) > 8.0) {
                    top_three.add(competitor);
                }
            }
            /* is equivalent (For loop) to
            for(int i=0; i<competitors.size(); i++) {
                if(competitors.get(i).getFinalScore(round) > 8.0) {
                    top_three.add(competitors.get(i));
                }
            }
             */
        }

        double min = top_three.get(0).getFinalScore(round); // initialization min
        int eliminate_index = 0; // initialization eliminate_index
        if(top_three.size() > 3) {
            for(int i=0; i<competitors.size(); i++) {
                if(top_three.get(i).getFinalScore(round)< min) {
                    eliminate_index = i;
                }
            }
            top_three.remove(eliminate_index);
        }
        return top_three;
    }
