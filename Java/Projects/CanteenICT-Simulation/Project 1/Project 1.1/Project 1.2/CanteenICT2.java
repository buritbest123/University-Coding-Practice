...
public class CanteenICT {
	...
	//******************************************************************//
	public List<Customer> waitToEnterQueuenext= new ArrayList<Customer>();//new waitToEnterQueue array is use to modify and try to not modify in the main array
	public List<Customer> waitToSeatQueuenext= new ArrayList<Customer>();//new waitToSeatQueue array is use to modify and try to not modify in the main array
	public List<Customer> getwaitToEnterQueue(){ 
		return this.waitToEnterQueue;
	}
	public List<FoodStall> getFoodlist(){return foodStalls;}
	...
	{	
		...
		//************************************** YOUR CODE HERE *********************************//
		int allp=0;   //find the installCost spend to open stall
		for(FoodStall a: this.foodStalls) {
			List<FoodStall.Menu> menu=a.getMenu();
			for(FoodStall.Menu dish:menu) {
				allp=allp+FoodStall.INSTALLATION_COST[dish.ordinal()];
			}
		}
	...
	public boolean validateCanteen()
	{
		...
		//************************************** YOUR CODE HERE *********************************//
		if(getInstallCost()>MAX_BUDGET){//check if the cost is over the MaxBudget return false
			return false;
		}
		if(tables.size()<1) {//if table less than 1 or is not available retuen false
			return false;
		}
		for(FoodStall a:foodStalls){// check that stall must have all kind of menu
			if(a.getMenu().size()==FoodStall.Menu.values().length) {
				return true;
			}
		}
		return false;
	...
	private void preprocess()
	{
		...
		this.waitToEnterQueuenext.removeAll(this.waitToEnterQueuenext);
	...
	{
		//******************************************** YOUR CODE HERE (IF ANY) *******************************//
		for(FoodStall nf : foodStalls){
			nf.getCustomerQueue().removeAll(nf.getCustomerQueue());
			nf.getCustomerQueue().addAll(nf.customerQueuenext);
		}
		this.waitToEnterQueue.removeAll(this.waitToEnterQueue);
		this.waitToEnterQueue.addAll(this.waitToEnterQueuenext);
		for(Table nt : tables){
			nt.getSeatedCustomers().removeAll(nt.getSeatedCustomers());
			nt.getSeatedCustomers().addAll(nt.seatedCustomersnext);
		}
	...
	public int setCustomers(String customerConfig)
	{
		...
		for(int i = 0; i < customerConfig.length(); i++)
		{
			...
			{	case 'D': c = new Customer(this); break;
				...
				case 'S': c = new Customer(this); break;
				case 'P': c = new Customer(this); break;
				case 'A': c = new Customer(this); break;
				case 'I': c = new Customer(this); break;
